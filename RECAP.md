...
### Fase 4 — Gestione container e download automatizzati (BLOCCO A ✅ 31/7 — BLOCCO B installato senza VPN 🟡 3-4/8/2026) → guida: [FASE4-DOWNLOAD.md](FASE4-DOWNLOAD.md)
...
2. [~] **Download automatizzato — installato il 3-4/8/2026 nella variante SENZA VPN** (debug a strati): qBittorrent (`:8080`) + Prowlarr (`:9696`) + Sonarr (`:8989`) + Radarr (`:7878`) + **Jellyseerr (`:5055`**, richieste stile Netflix, gira le richieste a Sonarr/Radarr) — compose unico in `~/docker/arr`. Collegamenti tutti verificati via API; **hardlink su CIFS OK** (import istantanei); test download diretto ok (ISO Debian); Connect Sonarr/Radarr→Jellyfin attivo. **Mancano**: VPN+gluetun (prerequisito per gli indexer veri), collaudo import (archive.org giù durante il test — in coda "Night of the Living Dead" 1968), riavvio di prova con lo stack completo
...
- Prossimo passo con Claude: **VPN per torrent** — analisi completa (se serve davvero, confronto AirVPN vs ProtonVPN, prezzi, raccomandazione): **[SCELTA-VPN.md](SCELTA-VPN.md)**, decisione in sospeso al 4/8 → poi **gluetun** davanti a qBittorrent (§4 della guida Fase 4: host download client da `qbittorrent` a `gluetun`) → **indexer veri** in Prowlarr. Nel frattempo: collaudo catena quando archive.org torna su (richiesta NOTLD in coda; ricerca manuale Prowlarr + Manual Import). Rifiniture: widget Homarr (integrazioni), test direct play Jellyfin al primo video, **riavvio di prova del NUC** (da fare quando il PC è sulla LAN di casa: da remoto niente WoL di riserva), eventuale Fase 5 (dominio)
